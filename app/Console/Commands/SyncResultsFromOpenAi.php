...
final class SyncResultsFromOpenAi extends Command
{
    protected $signature = 'world-cup:sync-results-openai
                            {--dry-run : Preview changes without persisting them}
                            {--dummy : Show raw OpenAI response and fixture preview without writing to the database}
                            {--data-only : Hit OpenAI directly and dump raw results, skipping all DB interaction}
                            {--specific-date= : With --data-only, restrict results to matches played on this date (YYYY-MM-DD)}';

    protected $description = 'Fetch World Cup 2026 match results from OpenAI and update fixtures';

    public function __construct(private readonly WorldCupResultsProviderInterface $provider)
    {
        parent::__construct();
    }
    ...
    public function handle(): int
    {
        if ($this->option('data-only')) {
            ...
            try {
                $raw = $this->provider->fetchRawResults($specificDate);
        ...
        try {
            $results = $this->provider->fetchResults($fixtures);
        } catch (RuntimeException $exception) {
            $this->error($exception->getMessage());
        ...
        if (! $dummy) {
            $this->table(['Home', 'Score', 'Away', 'OpenAI Status', 'Action'], $rows);
    ...
    {
        $this->newLine();
        $this->comment('=== RAW DATA RECEIVED FROM OPENAI ===');
        ...
        $this->table(['Fixture ID', 'Home Team', 'Home Score', 'Away Score', 'Away Team', 'Status'], $openAiRows);
